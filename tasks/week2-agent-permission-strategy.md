...
Human Check 不应该变成“每一步都问用户”。如果所有动作都要人工确认，Agent 的自动化价值会消失；如果所有动作都自动执行，资产风险又不可接受。

更合理的分层是：

| 类型 | 处理 |
|---|---|
| 低风险、可逆、小额、策略内 | 可自动执行并记录日志 |
| 中等风险 | 先 simulation，再让用户确认 |
| 高风险、不可逆、外部可见 | 必须展示影响并要求用户确认 |
| 超出 policy | 直接拒绝，不在执行时诱导用户临场突破规则 |

例如 policy 写明“每日最多 5 USDC”，当 Agent 想支付 6 USDC 时，系统不应该问“是否继续”，而应该返回：

```text
已拒绝：超过今日 5 USDC 额度。
如需调整额度，请进入权限设置修改 policy。
```

这个设计避免用户在高压或误导场景下临时打破自己预先设置的安全边界。

## Audit Log

每次 Agent 尝试执行链上动作，都应该留下可审计记录：

| 字段 | 作用 |
|---|---|
| 用户原始目标 | 事后判断 Agent 是否偏离任务 |
| Agent 生成的动作草稿 | 记录 AI 计划了什么 |
| policy 检查结果 | 证明是否越界 |
| simulation 摘要 | 说明预期资产变化 |
| 人工确认记录 | 证明用户在哪一步确认 |
| tx hash / event | 链上 proof |
| 失败原因 | 便于复盘和改进 |

Audit Log 不是为了好看，而是为了追责和复盘：Agent 做了什么、为什么做、有没有越界、失败在哪里，都应该可查。
